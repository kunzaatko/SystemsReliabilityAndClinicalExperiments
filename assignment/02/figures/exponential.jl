...
lines!(ax_hists_A, range(extrema(nomiss(A[!, "survt_c1"]))..., length=1000), x -> pdf(A_exp_MLE, x); color=col_wong[3], label=L"\hat{f}_\text{MLE}")

axislegend(ax_hists_A; position=:rt)

include("../save_fig.jl")(fig_hists_A, "histogram_vs_parametric_A", ".")
...
scatter!(ax_QQ_A, quantile.(A_exp_MLE, 1 .- risk.(A_KM, A_KM.times)), A_KM.times; color=col_wong[1])
ax_QQ_A.ylabel = L"t_{(j)}"
ax_QQ_A.xlabel = L"F_{\text{MLE (Exp)}}^{-1}(\hat{F}_\text{KM}(t_{(j)}))"
# qqplot!(ax_QQ_A, Exponential(1 / MLE_exponential_table[1, 2]), nomiss(A[!, "survt_c1"]))
axislegend(ax_QQ_A, position=:lt)

include("../save_fig.jl")(fig_QQ_A, "QQ_exponential_A", ".")
...
scatter!(ax_PP_A, 1 .- risk.(A_KM, A_KM.times), cdf.(B_exp_MLE, A_KM.times))
ax_PP_A.xlabel = L"\hat{P}_\text{KM}(t_{(j)})"
ax_PP_A.ylabel = L"\hat{P}_\text{MLE (Exp)}(t_{(j)})"
axislegend(ax_PP_A, position=:lt)

include("../save_fig.jl")(fig_PP_A, "PP_exponential_A", ".")
...
lines!(ax_hists_B, range(extrema(nomiss(B[!, "survt_c1"]))..., length=1000), x -> pdf(B_exp_MLE, x); color=col_wong[3], label=L"\hat{f}_\text{MLE}")

axislegend(ax_hists_B; position=:rt)

include("../save_fig.jl")(fig_hists_B, "histogram_vs_parametric_B", ".")
...
scatter!(ax_QQ_B, quantile.(B_exp_MLE, 1 .- risk.(B_KM, B_KM.times)), B_KM.times; color=col_wong[1])
ax_QQ_B.ylabel = L"t_{(j)}"
ax_QQ_B.xlabel = L"F_{\text{MLE (Exp)}}^{-1}(\hat{F}_\text{KM}(t_{(j)}))"
# qqplot!(ax_QQ_B, Exponential(1 / MLE_exponential_table[1, 2]), nomiss(B[!, "survt_c1"]))
axislegend(ax_QQ_B, position=:lt)

include("../save_fig.jl")(fig_QQ_B, "QQ_exponential_B", ".")
...
scatter!(ax_PP_B, 1 .- risk.(B_KM, B_KM.times), cdf.(B_exp_MLE, B_KM.times))
ax_PP_B.xlabel = L"\hat{P}_\text{KM}(t_{(j)})"
ax_PP_B.ylabel = L"\hat{P}_\text{MLE (Exp)}(t_{(j)})"
axislegend(ax_PP_B, position=:lt)

include("../save_fig.jl")(fig_PP_B, "PP_exponential_B", ".")
